...
🕯 HINT: Use the 'elem' function to check whether an elq2 ement belongs to a list
-}
contains :: Int -> [[Int]] -> [[Int]]
contains el l = filter (\sub_l -> el `elem` sub_l) l
...
rotate r l
  | r < 0 = []
  | otherwise = take (length l) (drop r (cycle l))
